...
static void R_line(struct graph *g, int particle_idx, int vertex_idx) {
	FPRINTF ("line(data[,%d], col=\"%s\", lwd=2)\n",
...
static void R_legend(struct graph *g) {
        ...
        FPRINTF("legend(xmax-100, ymax-0.1, legend=c(");
        for (i=0; i<g->n; i++) {
                FPRINTF("\"v%d\"", i+1);
                if (i != g->n-1)
                        FPRINTF(",");
...
static void R_do(struct graph *g) {
	int i, j;
...
static void fwrite_data(struct graph *g) {
	struct walker *w;
	struct vertex *v;
	int i, j, t;
	char *sep = "";
